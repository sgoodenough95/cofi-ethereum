...
contract SupplyFacet is Modifiers {
        ...
        returns (uint256 mintAfterFee)
    {
        ...
        LibVault._approve(s.vault[fiAsset], amount);
    ...
    /// @param  recipient       The recipient of the fiAssets.
    /// @param  referral        The referral account (address(0) if none provided).
    function underlyingViaPrimeToFi(
        uint256 amount,
        uint256 minAmountOut, // E.g., 1,000 * 0.9975 = 997.50. Auto-set to 0.25%.
        address fiAsset,
        address depositFrom,
        ...
        returns (uint256 mintAfterFee)
    {
        require(s.primeParams[s.vault[fiAsset]].enabled == 1, 'SupplyFacet: Prime not enabled');
        ...
        (bool success, ) = address(this).call(abi.encodeWithSelector(
            s.primeParams[s.vault[fiAsset]].toPrimeOp,
            fiAsset,
            amount
        ));
        require(success, 'SupplyFacet: Underlying to prime operation failed');
        require(s.RETURN_ASSETS > 0, 'SupplyFacet: Zero return assets received');
        LibVault._approve(s.vault[fiAsset], s.RETURN_ASSETS);

        (success, ) = address(this).call(abi.encodeWithSelector(
            s.primeParams[s.vault[fiAsset]].convertToUnderlyingOp,
            LibVault._getAssets(
                LibVault._wrap(
                    s.RETURN_ASSETS,
                    s.vault[fiAsset],
        ...
        returns (uint256 burnAfterFee)
    {
        ...
        // Before redeem hook (e.g., burn LP token).
    ...
    /// @param  depositFrom     The account to deposit fiAssets from.
    /// @param  recipient       The recipient of the underlyingAssets.
    function fiToUnderlyingViaPrime(
        uint256 amount,
        uint256 minAmountOut,
        address fiAsset,
        address depositFrom,
        ...
        returns (uint256 burnAfterFee)
    {
        ...
        (bool success, ) = address(this).call(abi.encodeWithSelector(
            s.primeParams[s.vault[fiAsset]].convertToPrimeOp,
        ...
        s.EXT_GUARD = 1;
        (success, ) = address(this).call(abi.encodeWithSelector(
            s.primeParams[s.vault[fiAsset]].toUnderlyingOp,
            fiAsset,
    ...
    function toPrime_HOPUSDCLP(
        address fiAsset,
        uint256 amount
    ) public EXTGuard {

        s.RETURN_ASSETS = LibVault._toPrime_HOPUSDCLP(fiAsset, amount);
    }
    ...
    function toUnderlying_HOPUSDCLP(
        ...
        s.RETURN_ASSETS = LibVault._toUnderlying_HOPUSDCLP(fiAsset, amount);
    }
    ...
    function convertToUnderlying_HOPUSDCLP(
        ...
        s.RETURN_ASSETS = LibVault._convertToUnderlying_HOPUSDCLP(amount);
    }

    function convertToPrime_HOPUSDCLP(
        uint256 amount
    ) public {

        s.RETURN_ASSETS = LibVault._convertToPrime_HOPUSDCLP(amount);
    }
