...
describe('First test', function() {
  ...
  async function deploy() {
    ...
    return {
      owner, signer, diamond, feeCollector, cofi, ethfi, btcfi, wyvUSDC, wyvETH, wyvBTC
    }
  }
  ...
  describe('SupplyFacet', function() {
    ...
    it('Should exchange USDC for COFI --x, rebase, and back again x--', async function() {
      ...
      const { owner, signer, diamond, feeCollector, cofi, wyvUSDC } = await loadFixture(deploy)

      const impersonatedSigner = await ethers.getImpersonatedSigner(usdcAccount);

      const _usdc = (await ethers.getContractAt(USDC_ABI, USDC)).connect(impersonatedSigner)
      ...
      console.log('t1 wyvUSDC Diamond bal: ' + await wyvUSDC.balanceOf(diamond.address))
      ...
      // wyvUSDC tokens held in Diamond
      console.log('t2 wyvUSDC Diamond bal: ' + await wyvUSDC.balanceOf(diamond.address))
      // COFI fee minted to fee collector
      console.log('t2 feeCollector COFI bal: ' + await cofi.balanceOf(feeCollector.address))
      // OP tokens held in wrapper
      console.log('t2 wyvUSDC OP bal: ' + await op.balanceOf(wyvUSDC.address))
      ...
      // wyvUSDC tokens held in Diamond
      console.log('t3 wyvUSDC Diamond bal: ' + await wyvUSDC.balanceOf(diamond.address))
      // COFI fee minted to fee collector
      console.log('t3 feeCollector COFI bal: ' + await cofi.balanceOf(feeCollector.address))
      // OP tokens held in wrapper
      console.log('t3 wyvUSDC OP bal: ' + await op.balanceOf(wyvUSDC.address))
    })
  })
})