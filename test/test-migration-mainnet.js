...
/* Deployed Vaults */
const wmooHopUSDCLP = "0xcA5b76A9139F902CAc90dff97F1629318788b5E7"
const wmooHopETHLP = "0x75eB006993743f2B17790282704C8f481F16CA83"
const waBTC = "0xB300f8131f686F0E288e77325a317E55Bfd29b4e"
const waUSDC = "0x87e459Ec386eF9361B531D608f9097acdd6F8687"
const waETH = "0x1e2B0B37F9F370068FeEA24E467368ad8456fE5F"
...
describe('First test', function() {
  ...
  async function deploy() {
    ...
    const usdcWhale = "0x63643d4dd9adbe65ace96811a9ec1945cfb0f2c4"
    const wethWhale = "0xb2862aa42a8d13900A809b5F64fa2082a943c9AA"
    ...
    return {
      usdcWhale, wethWhale, owner, signer, diamond, USDC, cofi, ethfi, btcfi, feeCollector
    } // ausdc, hopUSDCLP, hopUSDCLPRewards, etc.
  }
  ...
  describe('SupplyFacet', function() {
    ...
    it('Should migrate from waETH to wmooHopETHLP', async function() {

      const { wethWhale, owner, signer, feeCollector, diamond, ethfi } = await loadFixture(deploy)

      const _impersonatedSigner = await ethers.getImpersonatedSigner(wethWhale);

      const _weth = (await ethers.getContractAt(WETH_ABI, wETH)).connect(_impersonatedSigner)

      // Transfer owner 100 wETH
      await _weth.transfer(owner.address, '100000000000000000000')

      // Transfer Diamond 0.1 wETH buffer
      await _weth.transfer(diamond.address, '100000000000000000')

      const weth = (await ethers.getContractAt(WETH_ABI, wETH)).connect(signer)
      const wmooHopETHLP_ = (await ethers.getContractAt(WMOO_HOP_LP_ERC20_ABI, wmooHopETHLP)).connect(signer)
      const waeth = (await ethers.getContractAt('AaveV3ERC4626Reinvest', waETH)).connect(signer)

      // Approve Diamond spend
      await weth.approve(diamond.address, '100000000000000000000')
      ...
      // wmooHopETHLP tokens held in Diamond
      console.log('t1 wmooHopUSDCLP Diamond bal: ' + await wmooHopETHLP_.balanceOf(diamond.address))
      ...
      const ownerETHFIBal = await ethfi.balanceOf(owner.address)

      // Owner USDC bal
      console.log('t2 Owner wETH bal: ' + await weth.balanceOf(owner.address))
      // Owner COFI bal
      console.log('t2 Owner ETHFI bal: ' + ownerETHFIBal.toString())
      // wmooHopETHLP tokens held in Diamond
      console.log('t2 wmooHopUSDCLP Diamond bal: ' + await wmooHopETHLP_.balanceOf(diamond.address))
      // ETHFI fee minted to fee collector
      console.log('t2 feeCollector COFI bal: ' + await ethfi.balanceOf(feeCollector.address))
      // waETH tokens held in Diamond
      console.log('t2 waETH Diamond bal: ' + await waeth.balanceOf(diamond.address))

      // Convert back to wETH (redeem operation on FiToken contract skips approval check).
      await cofiMoney.fiToUnderlying(
        ownerETHFIBal.toString(),
        '100000000000000000000',   // Test slippage
        ethfi.address,
        owner.address,
        owner.address
      )
